...
@section('content')
    <section class="pt-custom pb-5">
        <div class="container-fluid px-4">
            ...
            <div class="mb-4">
                <h1 class="h3 fw-bold mb-1 text-dark">Sampah News</h1>
                <p class="text-muted small mb-0">Kelola dan pulihkan artikel berita yang telah dihapus.</p>
            </div>
            <div class="mb-4">
            ...
            <div class="card border-0 shadow-sm rounded-4 overflow-hidden mb-4">
                <div class="card-header bg-white border-0 p-4 pb-0">
                    <div class="row align-items-center justify-content-between g-3">
                        <div class="col-auto">
                            <h5 class="fw-bold mb-0 text-secondary fs-6"><i class="fa-solid fa-trash-arrow-up me-2 text-danger"></i>Daftar Sampah News</h5>
                        </div>
                        <div class="col-md-4">
                            <form class="d-flex" method="GET">
                                <div class="input-group input-group-sm">
                                    <input name="title" class="form-control rounded-start-pill ps-3 bg-light border-0" type="search" placeholder="Cari judul berita..."
                        ...
                        <table class="table table-hover align-middle mb-0">
                            ...
                            <tbody>
                                @forelse ($news as $item)
                                    <tr>
                                        <th scope="row" class="ps-3 text-muted fw-semibold">{{ ($news->firstItem() ?? 0) + $loop->index }}</th>
                                        <td>
                                            <span class="fw-semibold text-dark d-block">{{ $item->title }}</span>
                                            <small class="text-muted"><i class="fa-regular fa-user me-1"></i> {{ $item->author ?? 'Admin' }}</small>
                                        </td>
                                        <td class="text-center">
                                            @php $avgRating = collect($item->rating->pluck('rating_value'))->avg() ?? 0; @endphp
                                            <i class="fa-solid fa-star me-1 text-warning"></i> {{ number_format($avgRating, 1) }}
                                        </td>
                                        <td class="text-center">
                                            <div class="d-flex justify-content-center align-items-center gap-1">
                                                <a href="{{ route('detailTrashNews', ['slug' => $item->slug]) }}" class="text-info text-decoration-none px-2 py-1" title="Detail">
                                                    <i class="fa-solid fa-circle-info"></i>
                                                </a>
                                                <a href="{{ route('restoreNews', ['slug' => $item->slug]) }}" class="text-success text-decoration-none px-2 py-1 btn-restore" title="Pulihkan">
                                ...
                                @empty
                                    <tr>
                                        <td colspan="4" class="text-center py-5 text-muted">
                                            ...
                                            <span>Tidak ada data sampah news yang ditemukan.</span>
                                        </td>
                                    </tr>
                                @endforelse
                            </tbody>
